...
-- A database class which implements the items 
Crew = class()

function Crew:__init(connection)
	self.connection = connection
end

-- Change the behaviour of class indexing
function Crew:__index(item)
	if item <= #self then
		return StarshipPerson(self.connection, item)
	end
end

-- We have to just re-define the length definition to ipairs function will work with our class instance
function Crew:__len()
...
passenger = nil
for _, crewItem in ipairs(Crew(db)) do
